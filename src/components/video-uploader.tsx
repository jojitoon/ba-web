...
export default function MuxVideoUploader({
  ...
  const handleSuccess = useCallback(
    async (event: any) => {
      ...
      const uploadId = uploadUrl.split("/upload/")[1]?.split("?")[0];
      if (!uploadId) {
      ...
      setUploadedVideos((prev) => [
        ...prev,
        { id: uploadId, status: "success", assetId: uploadId, filename, size },
      ]);

      setSelectedFile(null);
      setUploadStatus("success");

      onUploadComplete?.(uploadId, "", size);
    },
    [selectedFile, onUploadComplete, onUploadError, uploadUrl]
  );
