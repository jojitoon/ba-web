...
function getPlaybackId(video: any): string | null {
  // First try direct muxPlaybackId
  if (video.muxPlaybackId) {
    return video.muxPlaybackId;
  }
  // Fallback: extract from playbackUrl if available
  if (video.playbackUrl) {
    // Match patterns like: https://stream.mux.com/PLAYBACK_ID.m3u8
    // The playbackId is between stream.mux.com/ and .m3u8
    const match = video.playbackUrl.match(
      /stream\.mux\.com\/([^\/\?]+?)(?:\.m3u8|$)/
    );
    if (match && match[1]) {
      return match[1];
