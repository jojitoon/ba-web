...
export default function ProjectPage() {
  ...
  useEffect(() => {
    ...
    const shouldCount = shouldCountView('project', projectId);
    if (shouldCount) {
      // Increment views in the project record
      incrementViews({ id: projectId as any })
        .then(() => {
          console.log('Project view incremented successfully');
        })
        .catch((error) => {
      ...
      trackEvent({
        ...
        .then(() => {
          console.log('Project view tracked in analytics');
        })
        .catch((error) => {
          console.error('Failed to track view event:', error);
        });
    } else {
      console.log('Project view not counted (within tracking window)');
    }
